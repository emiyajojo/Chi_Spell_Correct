...
# 判断当前行的某个tag是不是一个"有效实体chunk的结束tag"
def end_of_chunk(prev_tag, tag, prev_type, type_):
    chunk_end = False
    # 如果上一个字符标签是E, 或者S, 则结束标志位True
...
# 判断当前行的某个tag是不是一个"有效实体chunk的起始tag"
...
# 计算关键指标的主要函数, 通过calculate()来调用
def evaluate(iterable, options=None):
    # 如果没有传入配置信息, 则调用本文件内的parse_args()函数来获得初始化的配置信息
    if options is None:
        options = parse_args([])
    # 实例化计数器类的对象
    counts = EvalCounts()
